import os
import re
import urllib.parse  # Added to handle spaces and special characters
...
<style>
    ...
    .label {{ display: block; margin-bottom: 10px; font-weight: bold; color: #555; font-size: 11px; text-transform: uppercase; word-wrap: break-word; max-width: 300px; }}
...
def detect_dimensions(path, name):
    ...
    # Robust regex for filenames like "...-1024x90 (1).jpg"
...
for advertiser in sorted(os.listdir('.')):
    if os.path.isdir(advertiser) and not advertiser.startswith('.'):
        ...
        for item in sorted(os.listdir(advertiser)):
            ...
            # URL Encode the filename for the browser
            safe_item_name = urllib.parse.quote(item)
            ...
            if os.path.isdir(item_path):
                ...
                if os.path.exists(index_file):
                    ...
                    <div class="ad-box">
                        ...
                        <iframe src="{safe_item_name}/index.html" width="{w}" height="{h}" scrolling="no"></iframe>
                    </div>'''
            
            # CASE 2: Static Ads (Fixes the KIA issue)
                ...
                <div class="ad-box">
                    <span class="label">{item} ({w}x{h})</span>
                    <img src="{safe_item_name}" width="{w}" height="{h}" alt="{item}">
                </div>'''
